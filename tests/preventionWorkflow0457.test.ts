...
describe("0.4.57 prevention workflow contracts", () => {
  it("keeps prevention detail status sections in the specialist component", () => {
    ...
    for (const label of ["Prüfung und Ausgangslage", "Anforderung an den Arbeitgeber", "Reaktion des Arbeitgebers", "Maßnahmenklärung und Umsetzung", "Ergebnis / Abschluss"]) {
  ...
  it("wires prevention overview clicks into the case workbench node target", () => {
    ...
    expect(app).toContain("openCaseNode");
    expect(app).toContain("onOpenCaseNode={openCaseNode}");
