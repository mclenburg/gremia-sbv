...
describe("Patch-4-Cross-Platform- und Packaging-Vertrag", () => {
  ...
  it("verifiziert auch im taggebundenen Releaseweg Startfähigkeit und Plattformpfade", () => {
    ...
    expect(releaseWorkflow).toContain("xvfb-run -a npm run ${{ matrix.platform_check }}");
  });
  ...
  it("erzwingt Artefaktprüfung für das aktuelle Packaging und ignoriert Altartefakte", () => {
    ...
    expect(buildPlatform).toMatch(
      /runNpmScript\('native:rebuild:electron'\);\s*const packagingStartedAt = Date\.now\(\);\s*runNodeScript\('scripts\/run-electron-builder\.cjs', selected\.builderArgs\);\s*runNodeScript\('scripts\/verify-release-artifacts\.cjs', \[selected\.os, '--since', String\(packagingStartedAt\)\]\);/,
    );
