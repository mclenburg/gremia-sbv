...
describe('SBV-Steuerungsprotokolle Schema-Integration', () => {
  it('bleibt als Migration 0040 vorhanden, während 0042 Beteiligungsverstöße ergänzt', () => {
    ...
    expect(APP_SCHEMA_VERSION).toBe('0049');
    expect(latestMigration).toBe('0049');
  });
  ...
  it('nimmt Steuerungsprotokolle in Migration-Reparatur und Integritätsprüfung auf', () => {
    const migrationService = [
      'services/migrations/migrationInference.ts',
      'services/migrations/migrationProcessSchemasC.ts',
      'services/migrations/migrationRepairValidation.ts',
    ].map(read).join('\n');
