...
import { INSTRUCTIONAL_TEXT_TRIGGERS, VISIBLE_DESCRIPTION_MAX_CHARS, requiresHelpRegistryDecision, textPolicyDecision } from '../../../src/app/shared/help/helpTextPolicy';
...
describe('0.9.5-h Help-Dialog-Infrastruktur', () => {
  ...
  it('macht Hilfe nur per explizitem helpId sichtbar und lässt description standardmäßig unverändert sichtbar', () => {
    const form = ['IndustrialFormCore', 'IndustrialTextInputs', 'IndustrialDeferredInputs', 'IndustrialSelectionInputs'].map((name) => source(`src/app/shared/components/${name}.tsx`)).join('\n');
    const workbench = ['WorkbenchPanels', 'WorkbenchStructure', 'WorkbenchData'].map((name) => source(`src/app/shared/components/${name}.tsx`)).join('\n');
