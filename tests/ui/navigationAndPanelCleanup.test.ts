...
describe('Navigation und Panel-Cleanup 0.9.2', () => {
  it('gruppiert die Sidebar fachlich statt alle Module als flache Liste zu zeigen', () => {
    ...
    expect(modules.filter((module) => module.group === 'core').map((module) => module.id)).toEqual(['persons', 'cases', 'case_handover', 'deadlines', 'activity_journal', 'meetings', 'sbv_control']);
