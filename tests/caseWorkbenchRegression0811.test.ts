...
const casesViewRender = readFileSync("src/app/features/cases/CasesViewRender.tsx", "utf8");
...
describe("0.8.11 case workbench wiring regression", () => {
  ...
  it("keeps existing case workbench components wired in the extracted render module", () => {
    expect(casesView).toContain("CasesViewRender");
    expect(casesViewRender).toContain("CaseDetailPanel");
    expect(casesViewRender).toContain("CaseWorkbenchFooter");
    expect(casesViewRender).toContain("CaseRegister");
    expect(casesViewRender).toContain("CaseTreePanel");
    expect(casesViewRender).toContain("CaseCreateModal");
  });
});
