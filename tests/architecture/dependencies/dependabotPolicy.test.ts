...
describe('Dependabot-Leitplanken ohne zusätzliche GitHub-Actions-Kosten', () => {
  ...
  it('stellt einen lokalen Hygiene-Check für Dependabot-PRs bereit', () => {
    expect(packageJson.scripts['dependency:hygiene']).toContain('tests/architecture/dependencyHygiene.test.ts');
    expect(packageJson.scripts['dependency:hygiene']).toContain('tests/architecture/dependencies/dependabotPolicy.test.ts');
  });
});
