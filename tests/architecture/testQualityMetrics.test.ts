...
describe('reproduzierbare Testqualitätsmetriken', () => {
  ...
  it('liefert einen maschinenlesbaren Bericht und hält das versionierte Ratchet ein', () => {
    ...
    const baseline = JSON.parse(readFileSync('maintenance/test-quality/test-quality-baseline.json', 'utf8')) as {
      minimumTotalFiles: number;
      minimumAssertions: number;
    };
    expect(report.totalFiles).toBeGreaterThanOrEqual(baseline.minimumTotalFiles);
    expect(report.assertions).toBeGreaterThanOrEqual(baseline.minimumAssertions);
