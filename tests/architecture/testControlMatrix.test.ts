...
describe('Patch 3 test control contracts', () => {
  ...
  it('keeps security and backup behavior in the security suite', () => {
    ...
    expect(assignments.security).toContain('tests/security/vault/securityServiceBehavior.test.ts');
    expect(assignments.security).toContain('tests/security/backup/backupServiceBehavior.test.ts');
    expect(assignments.security).toContain('tests/platform/demo/demoModeBehavior.test.ts');
  });
