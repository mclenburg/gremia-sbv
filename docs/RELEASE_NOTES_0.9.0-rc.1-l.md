# Release Notes – Gremia.SBV 0.9.0-rc.1-l

Stand: 0.9.0-rc.1-l

## Status

`0.9.0-rc.1-l` ist der erste Release Candidate von Gremia.SBV. Dieser Stand friert die fachliche Funktionalität nach der 0.8.13-Härtungsphase ein und dient der technischen, datenschutzfachlichen und praktischen Evaluation vor einem stabilen Release.
...
- E2E-Abdeckung für Inline-Kurzbefehle ergänzt.
- Der Smoke-Test öffnet eine Fallakte, erfasst im großen Protokolltextfeld `/praev Arbeitsplatzgefährdung klären`, erwartet den Dialog „Prävention anlegen“ und prüft anschließend die Ersetzung im Textfeld.
- Damit wird abgesichert, dass Kurzbefehle in großen Textfeldern aktiv bleiben und die Bearbeitung nicht durch einen Render- oder Dialogfehler blockiert wird.

Diese Korrektur enthält keine neuen Fachfeatures, keine Cloud-Synchronisation und keine Änderung am AGPL-3.0-or-later-Lizenzmodell. Sie dient ausschließlich der Testabsicherung eines RC-kritischen Arbeitsflusses.


## Korrektur 0.9.0-rc.1-l – Fallaktennahe Inline-Kommandos ohne Doppelverarbeitung

- Fallnotiz-Textfelder lösen Inline-Kommandos jetzt über den fallaktennahen Controller aus; der globale Textbefehl-Controller öffnet dafür keinen zweiten Dialog mehr.
- Die Fallnotiz-Erfassung scannt beim Tippen nicht mehr den vollständigen Protokolltext, sondern nutzt das vom Textfeld gemeldete neu eingegebene Befehlstoken.
- Allgemeine große Textfelder behalten den globalen Controller; dessen Befehlsargumente werden getrimmt, damit keine führenden Leerzeichen in Dialogfelder übernommen werden.
- Der E2E-Smoke-Test verwendet mit `/praev` einen bestehenden fallaktenrelevanten Befehl aus der registrierten Befehlsliste und prüft explizit, dass kein globaler Aufgabendialog parallel geöffnet wird.
