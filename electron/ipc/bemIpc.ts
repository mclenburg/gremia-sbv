...
export function registerBemIpc(
  ...
  registerIpcHandler(ipcMain, "bem:steps", async () => BEM_STEPS);
  registerIpcHandler(ipcMain, "bem:list", async (_event, caseId?: unknown) =>
    bem().list(assertOptionalString(caseId, "bem:list", "Fall-ID", { maxLength: 120 })),
  );
  registerIpcHandler(ipcMain, "bem:dashboard", async () => bem().dashboardSummary());
  registerIpcHandler(ipcMain, "bem:create", async (_event, input: unknown) =>
    bem().create(assertRecordInput<CreateBemProcessInput>(input, "bem:create")),
  );
  registerIpcHandler(ipcMain, "bem:update", async (_event, id: unknown, input: unknown) =>
    bem().update(
      assertString(id, "bem:update", "BEM-ID", { minLength: 1, maxLength: 120 }),
      assertRecordInput<UpdateBemProcessInput>(input, "bem:update"),
    ),
  );
  registerIpcHandler(ipcMain, "bem:warnings", async (_event, id: unknown) => {
