...
import type { UpdateRetentionSettingsInput } from "../../src/domain/models/retention.model.js";
...
export function registerRetentionIpc(
  ...
  registerIpcHandler(ipcMain, IPC_CHANNELS.retentionDashboard, async () => services.retention().buildDashboard());
  registerIpcHandler(ipcMain, IPC_CHANNELS.retentionSettingsGet, async () => services.retention().getSettings());
  registerIpcHandler(ipcMain, IPC_CHANNELS.retentionSettingsUpdate, async (_event, input: unknown) =>
    services.retention().updateSettings(
