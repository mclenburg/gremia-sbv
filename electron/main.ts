...
function resolvePreloadPath(): string {
  ...
  if (!preloadPath) {
    throw new Error(`Gremia.SBV preload script not found. Checked: ${candidates.join(', ')}`);
  }
...
function resolveAppIconPath(): string {
  ...
  const candidates = app.isPackaged
    ? [
        path.join(electronProcess.resourcesPath ?? '', 'assets', 'icons', 'png', '512x512.png'),
        path.join(electronProcess.resourcesPath ?? '', 'assets', 'icons', 'png', '256x256.png'),
        path.join(electronProcess.resourcesPath ?? '', 'assets', 'icons', 'icon.png'),
  ...
  if (!match) {
    console.warn('Gremia.SBV app icon not found. Checked:', candidates.join(', '));
  }
...
function resolvePackagedIndexHtml(): string {
  ...
  const candidates = [
    ...
    path.join(electronProcess.resourcesPath ?? '', 'app.asar', 'dist', 'index.html'),
  ...
  if (!indexHtml) {
    throw new Error(`Gremia.SBV renderer index.html not found. Checked: ${candidates.join(', ')}`);
  }
...
function registerDiagnostics(win: BrowserWindow): void {
  ...
  win.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
    console.error('Gremia.SBV renderer load failed', { errorCode, errorDescription, validatedURL });
  });
...
async function createWindow(): Promise<void> {
  ...
  setTimeout(() => {
    if (!win.isDestroyed() && !win.isVisible()) {
      console.warn('Gremia.SBV window shown by fallback timer because ready-to-show did not fire.');
      win.show();
    }
  }, 3000);
  ...
  session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
    try {
      ...
      const allowed = ['file:', 'devtools:', 'http:'].includes(url.protocol) &&
