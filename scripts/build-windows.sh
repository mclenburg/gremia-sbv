...
if [[ "${OSTYPE:-}" != msys* && "${OSTYPE:-}" != cygwin* && "$(uname -s)" != MINGW* ]]; then
  echo "Hinweis: Windows-Builds auf Linux benötigen je nach electron-builder-Version Wine/Mono."
...
echo "  Gremia.SBV-<version>-win-x64.exe              (Installer/NSIS)"
echo "  Gremia.SBV-<version>-win-x64-portable.exe     (portable EXE, je nach electron-builder-Benennung)"
