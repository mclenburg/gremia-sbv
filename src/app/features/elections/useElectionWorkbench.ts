...
export function useElectionWorkbench(){
 ...
 async function run<T>(op:()=>Promise<T>,feedback:string|((result:T)=>ElectionFeedback)):Promise<T|undefined>{setError('');setNotice('');try{const result=await op();await refresh(selectedId);const resolved=typeof feedback==='function'?feedback(result):{message:feedback,tone:'success' as const};if(resolved.tone==='warning')setError(resolved.message);else setNotice(resolved.message);return result;}catch(e){setError(e instanceof Error?e.message:'Aktion fehlgeschlagen.');return undefined;}}
