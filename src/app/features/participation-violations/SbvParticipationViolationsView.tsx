...
import type { ParticipationViolationStage, ParticipationViolationType } from '../../core/models/sbv-participation-violation.model';
...
export function SbvParticipationViolationsView({ cases, onOpenJournalPrefill }: { cases: CaseRecord[]; onOpenJournalPrefill?: (prefill: ActivityJournalPrefill) => void }) {
  const state = useSbvParticipationViolations({ cases, onOpenJournalPrefill });
  ...
  return (
    <WorkbenchPage
      ...
      description="Strukturierte SBV-Protokollierung fehlender, verspäteter oder unvollständiger Beteiligung. Keine automatische Arbeitgeberkommunikation."
    >
      ...
      <WorkbenchGrid>
        <FormSection
          ...
          description="Dokumentiert werden Pflichtverstoß, richtiges Verfahren und die Eskalationsstufe. DOCX-Schreiben werden bewusst erzeugt und verschlüsselt abgelegt."
        ...
        >
          ...
          <div className="industrial-form-grid industrial-form-grid-auto">
            ...
            <SelectInput label="Fallbezug" value={state.form.caseId ?? ''} options={state.caseOptions} onValueChange={state.updateCaseContext} />
            <TextInput label="Kontext-ID" value={state.form.sourceContextId} required onValueChange={(sourceContextId) => state.updateForm({ sourceContextId })} />
            <TextInput label="Betreff" value={state.form.subject} required onValueChange={(subject) => state.updateForm({ subject })} />
            <TextInput label="Rechtsgrundlage" value={state.form.legalBasis ?? ''} onValueChange={(legalBasis) => state.updateForm({ legalBasis })} />
          </div>
          <TextareaInput label="Maßnahme / Sachverhalt" value={state.form.measureDescription} required onValueChange={(measureDescription) => state.updateForm({ measureDescription })} />
          <TextareaInput label="Was war falsch?" value={state.form.wrongBehavior} required onValueChange={(wrongBehavior) => state.updateForm({ wrongBehavior })} />
          <TextareaInput label="Was wäre richtig gewesen?" value={state.form.requiredBehavior} required onValueChange={(requiredBehavior) => state.updateForm({ requiredBehavior })} />
          ...
          <FormActions>
            <IndustrialButton disabled={state.busy} onClick={() => void state.createViolation()}>
              <Plus className="h-4 w-4" aria-hidden="true" /> Verstoß protokollieren
            </IndustrialButton>
          </FormActions>
        </FormSection>
        ...
        <FormSection
          ...
          description="Statusänderungen laufen über die Transition-Map und erzeugen Verlaufseinträge."
