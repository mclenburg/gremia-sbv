    ...
    <div className="industrial-modal-backdrop" role="dialog" aria-modal="true">
      <section className="industrial-modal">
        ...
        <div className="industrial-settings-form mt-5">
          ...
          {(draft.processType === 'prevention' || draft.processType === 'bem' || draft.processType === 'workplace_accommodation') && <label><span>{draft.processType === 'bem' ? 'Reaktionsfrist optional' : draft.processType === 'workplace_accommodation' ? 'Umsetzungs-/Wiedervorlage optional' : 'Frist Arbeitgeberreaktion optional'}</span><input type="datetime-local" value={draft.dueAt} onChange={(event) => update({ dueAt: event.target.value })} /></label>}
