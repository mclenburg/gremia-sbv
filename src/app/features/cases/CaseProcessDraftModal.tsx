    ...
    <div className="industrial-modal-backdrop" role="dialog" aria-modal="true">
      <section className="industrial-modal">
        ...
        <div className="industrial-settings-form mt-5">
          ...
          {draft.processType === 'prevention' && <label><span>Frist Arbeitgeberreaktion optional</span><input type="datetime-local" value={draft.dueAt} onChange={(event) => update({ dueAt: event.target.value })} /></label>}
          {draft.processType !== 'prevention' && <div className="industrial-message industrial-message-warning">Das Fachmodul ist noch nicht vollständig gebaut. Gremia.SBV legt deshalb zunächst eine fallbezogene Maßnahme als vertrauliche Notiz an.</div>}
        </div>
