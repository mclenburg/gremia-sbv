    ...
    <div className="industrial-modal-backdrop" role="presentation">
      <section className="industrial-modal" role="dialog" aria-modal="true" aria-labelledby="case-process-draft-title">
        ...
        <div className="industrial-settings-form">
          ...
          {(draft.processType === 'prevention' || draft.processType === 'bem' || draft.processType === 'workplace_accommodation') && <label><span>{draft.processType === 'bem' ? 'Reaktionsfrist optional' : draft.processType === 'workplace_accommodation' ? 'Umsetzungs-/Wiedervorlage optional' : 'Frist Arbeitgeberreaktion optional'}</span><input type="datetime-local" value={draft.dueAt} onChange={(event) => update({ dueAt: event.target.value })} className="industrial-input" /></label>}
          {draft.processType !== 'prevention' && draft.processType !== 'bem' && draft.processType !== 'participation' && draft.processType !== 'workplace_accommodation' && <div className="industrial-message industrial-message-warning">Das Fachmodul ist noch nicht vollständig gebaut. Gremia.SBV legt deshalb zunächst eine fallbezogene Maßnahme als vertrauliche Notiz an.</div>}
        </div>
