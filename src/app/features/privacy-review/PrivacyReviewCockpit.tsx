...
export function PrivacyReviewCockpit({ cases, onNavigate }: { cases: CaseRecord[]; onNavigate: (view: ViewId) => void }) {
  ...
  return <section className="industrial-card no-card-hover" aria-labelledby="privacy-review-title" data-e2e="privacy-review-cockpit">
    <div className="industrial-card-header">
      ...
      <IndustrialButton variant="secondary" disabled={busy} onClick={() => void reloadRetention()}><RefreshCw className="h-4 w-4" aria-hidden="true" /> Aktualisieren</IndustrialButton>
    </div>
    ...
    <div className="industrial-form-grid industrial-form-grid-2 mt-4">
      <label><span>Risiko filtern</span><select className="industrial-select" value={riskFilter} onChange={(event) => setRiskFilter(event.target.value as RiskFilter)}>
        <option value="all">Alle Risikostufen</option><option value="critical">Kritisch</option><option value="warning">Prüfen</option><option value="info">Hinweis</option>
      </select></label>
      <label><span>Prüfaufträge durchsuchen</span><input className="industrial-input" type="search" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
    </div>
    <div className="industrial-table-shell mt-4"><table className="industrial-table">
      ...
      <tbody>{candidates.map((candidate) => { const view = retentionCandidateTarget(candidate); return <tr key={candidate.id}>
        ...
        <td>{view ? <IndustrialButton variant="secondary" onClick={() => onNavigate(view)}>Vorgang öffnen</IndustrialButton> : '—'}</td>
      </tr>; })}
      {!candidates.length && <tr><td colSpan={6}>{dashboard ? 'Keine passenden Prüfaufträge offen.' : 'Prüfaufträge werden geladen …'}</td></tr>}</tbody>
    </table></div>
    <div className="mt-4"><RetentionCasePrivacyActions cases={cases} busy={busy} setBusy={setBusy} setError={setError} setMessage={setMessage} reloadRetention={reloadRetention} /></div>
    {error && <div className="industrial-message industrial-message-warning mt-4" role="alert">{error}</div>}
    {message && <div className="industrial-message industrial-message-ok mt-4" role="status">{message}</div>}
  </section>;
}
