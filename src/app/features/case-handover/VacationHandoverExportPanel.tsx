...
export function VacationHandoverExportPanel({ cases, onCompleted }: { cases: CaseRecord[]; onCompleted: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!caseIds.length) return setError('Bitte mindestens eine Fallakte für die Vertretung auswählen.');
    if (!targetRecipientToken.trim()) return setError('Bitte die Empfängerkennung der Vertretungsinstanz einfügen.');
    ...
    if (!validUntil.trim() || !expiresAt) return setError('Bitte das Ende der Urlaubsvertretung angeben.');
    if (new Date(expiresAt).getTime() <= Date.now()) return setError('Das Ende der Urlaubsvertretung muss in der Zukunft liegen.');
    setBusy(true);
    try {
      ...
      setPassphrase('');
      await onCompleted();
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Urlaubsübergabe konnte nicht exportiert werden.'); }
