...
type ParticipationUpdate = (input: UpdateParticipationInput) => void;

function ParticipationCheckMatrix({ process, update }: { process: ParticipationRecord; update: ParticipationUpdate }) {
...
function ParticipationGuidance({ process }: { process: ParticipationRecord }) {
  const escalation = getParticipationEscalationAdvice(process);
  const actionLabels = getParticipationActionLabels(process);
  const documents = getParticipationDocumentRequirements(process.measureType);
  return <>
    <section className={`participation-escalation-panel participation-escalation-${escalation.level}`} aria-label="Eskalationsbewertung der SBV-Beteiligung">
      <div className="participation-section-head"><div><p className="industrial-kicker">Handlungslinie</p><h3>{escalation.title}</h3></div><span>{escalation.level === "critical" ? "kritisch" : escalation.level === "warning" ? "prüfen" : "stabil"}</span></div>
      <p>{escalation.reason}</p><p><strong>Nächster sauberer Schritt:</strong> {escalation.nextStep}</p>
...
function ParticipationStructuredFields({ process, update }: { process: ParticipationRecord; update: ParticipationUpdate }) {
  return <div className="industrial-form-grid">
    ...
    <SelectInput label="Entscheidungsstand" value={process.decisionStage} options={decisionStageOrder.map((item) => ({ value: item, label: decisionStageLabels[item] }))} onValueChange={(value) => update({ decisionStage: value as ParticipationDecisionStage })} />
    <DeferredDateTimeInput label="Kenntnis der SBV" value={toDateTimeLocal(process.firstKnownAt)} onCommit={(value) => update({ firstKnownAt: fromDateTimeLocal(value) })} />
    <DeferredDateTimeInput label="Unterrichtung erhalten" value={toDateTimeLocal(process.informationReceivedAt)} onCommit={(value) => update({ informationReceivedAt: fromDateTimeLocal(value) })} />
    <DeferredDateTimeInput label="Stellungnahmefrist" value={toDateTimeLocal(process.statementDueAt)} onCommit={(value) => update({ statementDueAt: fromDateTimeLocal(value) })} />
    <DeferredDateTimeInput label="Aussetzung verlangt" value={toDateTimeLocal(process.suspensionRequestedAt)} onCommit={(value) => update({ suspensionRequestedAt: fromDateTimeLocal(value), status: value ? "aussetzung_verlangt" : process.status })} />
  </div>;
}
...
export function ParticipationProcessDetail({ process, onUpdate, caseRecord, onOpenViolationPrefill }: {
  ...
  return <MeasureDetailFrame typeLabel="SBV-Beteiligung" title={process.title} statusLabel={statusLabels[process.status]} riskLevel={process.riskLevel} riskLabel={riskLabels[process.riskLevel]} summary={`§ 178 Abs. 2 SGB IX · ${measureLabels[process.measureType]} · Cockpit nur zur Übersicht`} nextStep={process.nextStep} requiresFollowUp={!process.informationComplete || !process.hearingBeforeDecision} actions={<div className="industrial-search-actions"><ActivityJournalContextButton context={{ contextType: "sbv_participation", contextId: process.id, caseId: process.caseId, title: process.title }} compact />{onOpenViolationPrefill && <ToolbarButton onClick={() => onOpenViolationPrefill(buildParticipationViolationPrefillFromMeasure(process, caseRecord))}><FileWarning className="h-4 w-4" aria-hidden="true" /> Beteiligungsverstoß dokumentieren</ToolbarButton>}</div>}>
    <div className="participation-case-detail">
      ...
      <ParticipationNarrativeFields process={process} update={update} />
    </div>
  </MeasureDetailFrame>;
}
