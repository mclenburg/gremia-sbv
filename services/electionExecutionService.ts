...
export class ElectionExecutionService {
  constructor(
    ...
    private readonly deadlines = new DeadlineService(db),
  ...
  saveMailBallot(electionId: string, input: SaveElectionMailBallotInput): ElectionMailBallotRecord {
    ...
    return new DatabaseUnitOfWork(this.db).run(() => {
      ...
      recordElectionEvent(this.db, electionId, 'mail_ballot_updated', { late: Boolean(input.lateReceivedAt), declarationValid: input.declarationValid ?? null });
  ...
  recordElectionDayChecklist(electionId: string, input: ElectionDayChecklistInput): ElectionExecutionOverview {
    assertElectionExists(this.db, electionId);
    recordElectionEvent(this.db, electionId, 'election_day_checklist', {
  ...
  recordTotals(electionId: string, input: RecordElectionTotalsInput): ElectionExecutionOverview {
    ...
    const maxSelections = input.officeType === 'representative' ? 1 : electionDeputyCount(this.db, electionId);
    ...
    return new DatabaseUnitOfWork(this.db).run(() => {
      ...
      const candidateIds = new Set(this.db.prepare<{ id: string }>(
      ...
      this.repository().replaceVoteTotals(electionId, input.officeType, rankings, electionDeputyCount(this.db, electionId));
      recordElectionEvent(this.db, electionId, 'count_recorded', {
        officeType: input.officeType,
  ...
  recordLotDecision(electionId: string, input: RecordElectionLotInput): ElectionResultRecord {
    ...
    return new DatabaseUnitOfWork(this.db).run(() => {
      ...
      recordElectionEvent(this.db, electionId, 'lot_decision', {
        officeType: input.officeType,
        rank: selectedRank,
  ...
  recordAcceptance(electionId: string, input: RecordElectionAcceptanceInput): ElectionExecutionOverview {
    ...
    return new DatabaseUnitOfWork(this.db).run(() => {
      ...
      const pending = this.db.prepare<{ count: number }>(`
      ...
      const unresolvedLots = this.db.prepare<{ count: number }>(`
        SELECT COUNT(*) AS count FROM sbv_election_results
      ...
      this.db.prepare('UPDATE sbv_elections SET status=?,updated_at=? WHERE id=?')
        .run(nextStatus, electionNow(), electionId);
      recordElectionEvent(this.db, electionId, 'acceptance_updated', {
  ...
  close(electionId: string, input: ElectionCloseInput): void {
    ...
    new DatabaseUnitOfWork(this.db).run(() => {
      ...
      recordElectionEvent(this.db, electionId, 'election_closed', {
  ...
  private ensureEmployerNotificationFollowUp(electionId: string): void {
    const election = this.db.prepare<{ office_term_end: string | null }>(
  ...
  private repository(): ElectionExecutionRepository { return new ElectionExecutionRepository(this.db); }

  private deadlineCoordinator(): ElectionExecutionDeadlineCoordinator {
    return new ElectionExecutionDeadlineCoordinator(this.db, this.deadlines, this.deadlinePolicy);
  }
