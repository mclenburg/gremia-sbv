...
export class RetentionLegalHoldService {
  constructor(private readonly database: DatabaseAdapter, private readonly owners = new RetentionOwnerRegistry()) {}

  place(owner: RetentionOwnerRef, reasonKey: string, legalReference?: string, untilAt?: string): RetentionLegalHoldRecord {
    if (!this.owners.exists(this.database, owner)) throw new Error('Legal Hold kann nur für einen vorhandenen Amtsvorgang gesetzt werden.');
    ...
    new DatabaseUnitOfWork(this.database).run(() => {
      ...
      new PersonalDataAuditLogService(this.database).append(auditRetentionLegalHoldChanged({
        action: 'create', holdId: id, ownerType: owner.type, ownerId: owner.id, reasonKey: normalizedReason,
      }));
    });
    const row = this.database.prepare<Row>('SELECT * FROM sbv_retention_legal_holds WHERE id = ?').get(id);
  ...
  release(holdId: string, releaseReason: string): RetentionLegalHoldRecord {
    ...
    const current = this.database.prepare<Row>('SELECT * FROM sbv_retention_legal_holds WHERE id = ?').get(holdId);
    if (!current) throw new Error('Legal Hold wurde nicht gefunden.');
    if (current.released_at) return mapRow(current);
    const now = new Date().toISOString();
    new DatabaseUnitOfWork(this.database).run(() => {
      this.database.prepare('UPDATE sbv_retention_legal_holds SET released_at = ?, release_reason = ?, updated_at = ? WHERE id = ?')
    ...
    const updated = this.database.prepare<Row>('SELECT * FROM sbv_retention_legal_holds WHERE id = ?').get(holdId);
