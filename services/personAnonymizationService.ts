...
export class PersonAnonymizationService {
  ...
  anonymizeStructuredPersonData(id: string, reason: string): PersonAnonymizationResult {
    ...
    const directlyLinkedRows = this.database.prepare<{ id: string }>('SELECT id FROM cases WHERE protected_person_id = ?').all(id);
    const person = service.anonymizeStructuredData(id, normalizedReason);
    const reviewService = new PrivacyReviewService(this.database);
  ...
  deleteStructuredPersonData(id: string, reason: string): { ok: true; affectedCaseIds: string[]; deletedPersonId: string } {
    ...
    this.database.prepare(`UPDATE person_case_links SET link_state = 'removed', anonymized_at = ?, link_reason = COALESCE(link_reason, ?) WHERE protected_person_id = ? AND link_state = 'active'`)
