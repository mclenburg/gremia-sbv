...
function accommodationStatusToMeasureStatus(status: WorkplaceAccommodationStatus): 'open' | 'in_progress' | 'waiting' | 'completed' | 'follow_up_required' {
  if (status === 'abgeschlossen') return 'completed';
  if (status === 'unterlagen_fehlen' || status === 'arbeitgeber_lehnt_ab' || status === 'eskaliert') return 'follow_up_required';
  if (status === 'angefragt' || status === 'in_pruefung') return 'waiting';
  if (status === 'in_umsetzung' || status === 'wirksamkeitspruefung' || status === 'inklusionsamt_einbezogen' || status === 'bewilligt') return 'in_progress';
  return 'open';
}
...
function mapRecord(row: any): WorkplaceAccommodationRecord {
  return {
    ...
    organizationalAdjustmentNeeded: toBool(row.organizational_adjustment_needed),
    ...
    homeofficeOrMobileWorkRelevant: toBool(row.homeoffice_or_mobile_work_relevant),
...
export function evaluateWorkplaceAccommodationWarnings(record: WorkplaceAccommodationRecord): WorkplaceAccommodationWarning[] {
  const warnings: WorkplaceAccommodationWarning[] = [];
  if (!record.requestedAdjustment.trim()) {
    warnings.push({ level: 'warning', message: 'Die gewünschte behinderungsgerechte Gestaltung ist noch nicht konkret beschrieben.' });
  }
  if (record.employerResponseStatus === 'offen' && record.status !== 'entwurf' && record.status !== 'abgeschlossen') {
    warnings.push({ level: record.riskLevel === 'kritisch' ? 'critical' : 'warning', message: 'Eine Arbeitgeberreaktion ist noch offen. Wiedervorlage und Unterlagenanforderung prüfen.' });
  }
  if (record.status === 'arbeitgeber_lehnt_ab' && !record.inclusionOfficeInvolved) {
    warnings.push({ level: 'critical', message: 'Arbeitgeber lehnt ab. Einschaltung des Inklusionsamts bzw. Beratung nach § 185 SGB IX prüfen.' });
  }
  if (record.implementationDueAt && new Date(record.implementationDueAt) < new Date() && !['umgesetzt', 'nicht_mehr_erforderlich'].includes(record.implementationStatus)) {
    warnings.push({ level: 'critical', message: 'Die dokumentierte Umsetzungsfrist ist überschritten.' });
  }
  if (record.effectivenessReviewAt && new Date(record.effectivenessReviewAt) < new Date() && record.status === 'wirksamkeitspruefung') {
    warnings.push({ level: 'warning', message: 'Die Wirksamkeitsprüfung ist fällig.' });
  }
  return warnings;
}
...
export class WorkplaceAccommodationService {
  ...
  private audit(action: Parameters<PersonalDataAuditLogService['append']>[0]['action'], subjectId: string | undefined, caseId: string | undefined, purpose: string): void {
    try {
      new PersonalDataAuditLogService(this.db).append({ action, subjectType: 'case_measure_workplace_accommodation', subjectId, caseId, purpose });
    } catch (error) {
      console.warn('Gremia.SBV workplace accommodation audit write failed', error);
    }
  }
  ...
  list(caseId?: string): WorkplaceAccommodationRecord[] {
    this.audit('read', undefined, caseId, caseId ? 'Arbeitsplatzgestaltungsmaßnahmen einer Fallakte anzeigen' : 'Arbeitsplatzgestaltungscockpit anzeigen');
  ...
  dashboardSummary(): WorkplaceAccommodationDashboardSummary {
    ...
    const now = new Date();
    return {
      open: rows.filter((row) => !['abgeschlossen'].includes(row.status)).length,
      critical: rows.filter((row) => row.riskLevel === 'kritisch' || evaluateWorkplaceAccommodationWarnings(row).some((warning) => warning.level === 'critical')).length,
      employerResponseOpen: rows.filter((row) => row.employerResponseStatus === 'offen' && row.status !== 'abgeschlossen').length,
      effectivenessReviewDue: rows.filter((row) => row.effectivenessReviewAt && new Date(row.effectivenessReviewAt) <= now && row.status === 'wirksamkeitspruefung').length
    };
  }
  ...
  getById(id: string): WorkplaceAccommodationRecord | undefined {
    const row = this.db.prepare<any>(`
      ...
      WHERE cm.id = ?
    `).get(id);
    if (row) this.audit('read', id, row.case_id, 'Arbeitsplatzgestaltung Detail anzeigen');
    return row ? mapRecord(row) : undefined;
  }

  create(input: CreateWorkplaceAccommodationInput): WorkplaceAccommodationRecord {
    if (!input.caseId) throw new Error('Arbeitsplatzgestaltung muss aus einer Fallakte heraus angelegt werden.');
    if (!input.title?.trim()) throw new Error('Arbeitsplatzgestaltung benötigt einen Titel.');
    ...
    const measure = new CaseMeasureService(this.db).create({
      ...
      createdFrom: 'manual',
      summary: input.requestedAdjustment || input.barrierOrLimitation || undefined,
      nextStep: input.nextStep || undefined,
      dueAt: input.implementationDueAt || input.effectivenessReviewAt,
      openedAt: timestamp,
      requiresFollowUp: status !== 'abgeschlossen'
    });
    ...
    this.event(measure.id, 'created', 'Arbeitsplatzgestaltung angelegt', input.title);
    ...
    if (input.createDefaultDeadlines !== false && implementationDueAt) {
      new DeadlineService(this.db).create({
        ...
        description: 'Automatische Wiedervorlage aus der fallaktenbezogenen Arbeitsplatzgestaltung.',
    ...
    if (input.createDefaultDeadlines !== false && effectivenessReviewAt) {
      new DeadlineService(this.db).create({
        ...
        description: 'Wiedervorlage zur Prüfung, ob die vereinbarte Arbeitsplatzgestaltung wirksam ist.',
        dueAt: effectivenessReviewAt,
        legalBasis: input.legalBasis?.trim() || '§ 164 Abs. 4 SGB IX',
        sourceEvent: 'case_measure_workplace_accommodation_effectiveness_review',
        severity: 'important',
        calculationMode: 'workflow',
        isLegalDeadline: false,
    ...
    this.audit('create', measure.id, input.caseId, 'Arbeitsplatzgestaltung in Fallakte angelegt');
    return this.getById(measure.id)!;
  }

  update(id: string, input: UpdateWorkplaceAccommodationInput): WorkplaceAccommodationRecord {
    const existing = this.getById(id);
    if (!existing) throw new Error(`Arbeitsplatzgestaltung nicht gefunden: ${id}`);
    const timestamp = nowIso();
    ...
    new CaseMeasureService(this.db).update(id, {
      ...
      summary: input.requestedAdjustment !== undefined ? input.requestedAdjustment : existing.requestedAdjustment,
      nextStep: input.nextStep !== undefined ? input.nextStep : existing.nextStep,
      dueAt: input.implementationDueAt !== undefined ? input.implementationDueAt : (input.effectivenessReviewAt !== undefined ? input.effectivenessReviewAt : existing.implementationDueAt ?? existing.effectivenessReviewAt),
    ...
    `).run(
      ...
      nextStatus,
      input.requestedAdjustment !== undefined ? input.requestedAdjustment : existing.requestedAdjustment,
      input.legalBasis !== undefined ? input.legalBasis : existing.legalBasis,
      input.barrierOrLimitation !== undefined ? input.barrierOrLimitation : existing.barrierOrLimitation ?? null,
      input.workplaceContext !== undefined ? input.workplaceContext : existing.workplaceContext ?? null,
      input.proposedSolution !== undefined ? input.proposedSolution : existing.proposedSolution ?? null,
      input.technicalAidNeeded !== undefined ? boolToInt(input.technicalAidNeeded) : boolToInt(existing.technicalAidNeeded),
      input.organizationalAdjustmentNeeded !== undefined ? boolToInt(input.organizationalAdjustmentNeeded) : boolToInt(existing.organizationalAdjustmentNeeded),
      input.workingTimeAdjustmentNeeded !== undefined ? boolToInt(input.workingTimeAdjustmentNeeded) : boolToInt(existing.workingTimeAdjustmentNeeded),
      input.qualificationNeeded !== undefined ? boolToInt(input.qualificationNeeded) : boolToInt(existing.qualificationNeeded),
      input.fixedWorkplaceNeeded !== undefined ? boolToInt(input.fixedWorkplaceNeeded) : boolToInt(existing.fixedWorkplaceNeeded),
      input.homeofficeOrMobileWorkRelevant !== undefined ? boolToInt(input.homeofficeOrMobileWorkRelevant) : boolToInt(existing.homeofficeOrMobileWorkRelevant),
      ...
      input.employerResponseAt !== undefined ? toIso(input.employerResponseAt) : existing.employerResponseAt ?? null,
      input.implementationStatus ?? existing.implementationStatus,
      input.implementationDueAt !== undefined ? toIso(input.implementationDueAt) : existing.implementationDueAt ?? null,
      input.effectivenessReviewAt !== undefined ? toIso(input.effectivenessReviewAt) : existing.effectivenessReviewAt ?? null,
      input.outcome !== undefined ? input.outcome : existing.outcome ?? null,
      timestamp,
      id
    );

    this.event(id, 'updated', 'Arbeitsplatzgestaltung aktualisiert', JSON.stringify(input));
    this.audit('update', id, existing.caseId, 'Arbeitsplatzgestaltung geändert');
